# Proxima Protocol dapp setup script
# Run this from inside your proxima-protocol project folder (PowerShell)
...
'@
Set-Content -Path "app/providers.tsx" -Value $content -NoNewline -Encoding UTF8
...
'@
Set-Content -Path "components/ProximaProtocolApp.tsx" -Value $content -NoNewline -Encoding UTF8
...
'@
Set-Content -Path "lib/chains.ts" -Value $content -NoNewline -Encoding UTF8
...
'@
Set-Content -Path "lib/wagmi.ts" -Value $content -NoNewline -Encoding UTF8
...
'@
Set-Content -Path "lib/contract.ts" -Value $content -NoNewline -Encoding UTF8

$content = @'
legacy-peer-deps=true
'@
Set-Content -Path ".npmrc" -Value $content -NoNewline -Encoding UTF8
...
'@
Set-Content -Path "app/page.tsx" -Value $content -NoNewline -Encoding UTF8
...
'@
Set-Content -Path "app/layout.tsx" -Value $content -NoNewline -Encoding UTF8

$content = @'
NEXT_PUBLIC_WALLETCONNECT_PROJECT_ID=PASTE_YOUR_REOWN_PROJECT_ID_HERE
'@
Set-Content -Path ".env.local" -Value $content -NoNewline -Encoding UTF8

Write-Host ""
Write-Host "Done. Files written:" -ForegroundColor Green
Write-Host "  app/providers.tsx"
Write-Host "  components/ProximaProtocolApp.tsx"
Write-Host "  lib/chains.ts"
Write-Host "  lib/wagmi.ts"
Write-Host "  lib/contract.ts"
Write-Host "  .npmrc"
Write-Host "  app/page.tsx"
Write-Host "  app/layout.tsx"
Write-Host "  .env.local"
Write-Host ""
Write-Host "IMPORTANT: edit .env.local and replace PASTE_YOUR_REOWN_PROJECT_ID_HERE with your real WalletConnect project ID." -ForegroundColor Yellow
Write-Host "IMPORTANT: edit components/ProximaProtocolApp.tsx, find DEPLOY_BLOCK = 0n and replace 0n with the real deployment block number." -ForegroundColor Yellow
